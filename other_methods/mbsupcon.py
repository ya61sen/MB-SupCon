...
def train(epoch, model, criterion_gm, factor_label_train, optimizer, train_loader, n_out_features,
    ...
    for idx, (data, index) in enumerate(train_loader):
        ...
        loss = criterion_gm(features.view(batch_size, len(model.keys), n_out_features), labels_train)
...
class MbSupConModel:
    ...
    def initialize(self, net_dict, split_by_subjects=False, subject_col_name=None, 
    ...
    def train_model(self, n_epoch=1000, gradient_clip=3, print_hist=True):
        ...
        factor_label_train = self.le.transform(self.df_with_covariates.loc[indexes_train, self.covariate].values)
        factor_label_val = self.le.transform(self.df_with_covariates.loc[indexes_val, self.covariate].values)
        ...
        for epoch in range(n_epoch):
            # Train
            loss = train(epoch, self.model, self.criterion_gm, factor_label_train, self.optimizer, 
            ...
            with torch.no_grad():
                for idx, (data, index) in enumerate(self.val_loader):
                    ...
                    labels_val = torch.tensor(labels_val).to(self.device)
                    for _ in data.keys():
    ...
    def _plot_history(self, history, plot_attr, plot_labels, ylabel, title, save_name=None, 
        ...
        if save:
            ...
            figure_save_folder = os.path.join(self.root_folder, 'figures/train_MB-SupCon/{}'.format(self.covariate))
    ...
    def save_training(self):
        ...
        torch.save({
            ...
            os.path.join(model_save_folder, 'MB-SupCon_{}_epoch-{}_temp-{}_dropout-{}_SGD-wd-{}_seed-{}.pth'.\
    ...
    def _split_Xy_original(self, which_dataset_list=['train', 'val', 'test']):
    ...
    def dim_reduction_embedding(self, dim_reduction_method, dim_reduction_random_seed=123, fontsize=20,
        ...
        for omics_key in self.keys:
            ...
            for which_dataset in which_dataset_list:
                ...
                self._scatter_2d_cate(score=omics_trans, covariate_value=_y, fontsize=fontsize,
                             title='{} on MB-SupCon embedding\n{} | {}'.format(dim_reduction_method.upper(), 
                                                                 omics_key, which_dataset.capitalize()),
                             save_folder_name='{}_embedding/MB-SupCon_{}_epoch-{}_temp-{}_dropout-{}_SGD-wd-{}_seed-{}'.\
        ...
        dim_reduction_save_folder = os.path.join(self.root_folder, 
                                             'outputs/{}_embedding/MB-SupCon_{}_epoch-{}_temp-{}_dropout-{}_SGD-wd-{}_seed-{}'.\
                                             format(dim_reduction_method, self.covariate, self.n_epoch, 
                                             self.temperature, self.dropout_rate, self.weight_decay, self.random_seed))
    ...
    def dim_reduction_original(self, dim_reduction_method, dim_reduction_random_seed=123, fontsize=20,
                               which_dataset_list=['train', 'val', 'test']):
        dim_reduction_pipeline = self._dim_reduction_pipeline(dim_reduction_method=dim_reduction_method,
                                                dim_reduction_random_seed=dim_reduction_random_seed)
        split_Xy_original_dict = self._split_Xy_original(which_dataset_list=which_dataset_list)
